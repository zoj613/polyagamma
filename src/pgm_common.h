 ...
 *
 * - if `scaled` is true, the scaled complementary error function is returned.
 *
 * References
 * ----------
...
{
    static const double max_val = 2.535599352761576e+307;
    static const double huge_val = 47453132.81212577;
    ...
    if (x < small_val) {
       ...
       return 1;
        ...
        double z = x * x;
        double r = x * ((((p4 * z + p3) * z + p2) * z + p1) * z + p0) /
                        ((((z + q3) * z + q2) * z + q1) * z + q0);
    ...
    else if ((scaled && x > max_val) || (!scaled && x > big_val)) {
        return 0;
    }
    else if (scaled && x > huge_val) {
        return PGM_1_SQRTPI / x;
    }
    double z = x * x;
    double y = scaled ? 1 : exp(-z);

    if (!scaled && (x * DBL_MIN > y * PGM_1_SQRTPI)) {
        return 0;
    }
    else {
...
inverse_gaussian_cdf(double x, double mu, double lambda)
{
    ...
    return 0.5 * (pgm_erfc(a - b, false) + c * pgm_erfc(b + a, false) * c);
}
